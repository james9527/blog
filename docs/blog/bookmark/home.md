---
sidebar: auto
---
# 常用网址

## 底层技术
  + [tc39/ecma262](https://github.com/tc39/ecma262)
  + [v8](https://github.com/v8/v8)
  + [chromium](https://github.com/chromium/chromium)

## 语言标准
  + [ECMAScript中文文档](https://ecma262.docschina.org/)
  + [GraphQL](http://graphql.cn/)
  + [TS中文网](https://tslang.cn/)
  + [TypeScript中文文档](https://zhongsp.gitbooks.io/typescript-handbook/)

## 前端工程化相关
  + [webpack 中文文档(@印记中文)](https://webpack.docschina.org/)
  + [深入浅出Webpack ](http://webpack.wuhaolin.cn/)
  + [webpack详解](https://juejin.im/post/5aa3d2056fb9a028c36868aa)
  + [Babel -- 下一代JavaScript编译器](https://www.babeljs.cn/)
  + [Gulp中文网](https://www.gulpjs.com.cn/)
  + [Lerna -- 用于管理有多个包的 JavaScript 项目的工具](https://lernajs.io/)

## 编程思想
  + [了解BFF架构](https://segmentfault.com/a/1190000009558309)
  
## Vue相关
  + [Vue技术内幕](http://hcysun.me/vue-design/)
  + [Vue.js 技术揭秘](https://ustbhuangyi.github.io/vue-analysis/)
  + [A Vue-based server-side rendering framework](https://vapperjs.org/zh/)

## React(Native)相关
  + [React官网](https://reactjs.org/)
  + [React小书](http://huziketang.mangojuice.top/books/react/)
  + [Redux中文文档](http://cn.redux.js.org/)
  + [深入研究React setState的工作机制](https://www.cnblogs.com/danceonbeat/p/6993674.html)
  + [ReactNative知识小集](https://zhuanlan.zhihu.com/react-native-tips?from=groupmessage&isappinstalled=0)
  + [Hooks.guide](https://www.hooks.guide/)
  + [实现一个 Virtual DOM 算法](https://github.com/livoras/blog/issues/13)

## Node.js相关
  + [Node.js中文文档](http://nodejs.cn/api/)
  + [Koa -- 新一代node.js开发框架](https://koajs.docschina.org/)
  + [Node.js 框架](https://thinkjs.org/doc/index.html)
  + [深入理解node](https://github.com/yjhjstz/deep-into-node)

## 多端统一框架
  + [Taro](https://nervjs.github.io/taro/)

## Javascript原理相关
  + [解读 JavaScript 之引擎、运行时和堆栈调用](https://www.oschina.net/translate/how-does-javascript-actually-work-part-1)

## HTML5&CSS3相关
  + [查JS或CSS浏览器兼容性](https://caniuse.com/)
  + [Js in Css](https://css-tricks.com/the-css-paint-api/)
  + [animate.css](http://daneden.github.io/animate.css/)
  + [JavaScript和CSS3 动效库大全](https://www.ui.cn/detail/16348.html)

## 前端性能优化相关
  + [前端性能——监控起步](https://www.cnblogs.com/chuaWeb/p/PerformanceMonitoring.html)
  + [利用Webpack做web性能优化](http://www.open-open.com/lib/view/open1519653292388.html)
  + [自动化生成 H5 骨架页面](https://zhuanlan.zhihu.com/p/34702561)
  + [腾讯祭出大招VasSonic，让你的H5页面首屏秒开](https://segmentfault.com/a/1190000010711024)

## 数据相关（API）
  + [GraphQL](http://graphql.cn/)
  + [orm2 中文文档](https://wizardforcel.gitbooks.io/orm2-doc-zh-cn/content/)

## 协同工作相关
  + [常用fontIcons](http://www.fontawesome.com.cn/faicons/)
  + [产品设计协作平台-蓝湖](https://lanhuapp.com/web/#/item)
  + [在线压缩图片](https://tinypng.com/)
  + [Create and share beautiful images of your source code](https://carbon.now.sh)
  + [持续集成--Centos服务器配置](https://segmentfault.com/a/1190000013242438)

## 代码规范
  + [Eslint中文文档](https://cn.eslint.org/)

## 前端周边技术
  + [Docker镜像推送（push）到Docker Hub](https://blog.csdn.net/boonya/article/details/74906927)

## 前端社区
  + [MDN前端社区--Web技术文档](https://developer.mozilla.org/zh-CN/docs/Web)
  + [印记中文 - 挖掘国外前端新领域，为前端开发人员提供优质文档！](https://docschina.org/)
  + [Node.js中文社区](https://cnodejs.org/)
  + [leetcode-算法题库](https://leetcode-cn.com/problemset/all/)

## 啥是Python(Peiqi)
  + [零基础入门学习Python](https://edu.aliyun.com/course/137)
  + [Python基础快速入门实战教程](https://edu.aliyun.com/course/154)
  + [Python开发实战系列(1)：基础入门](https://edu.aliyun.com/course/1370)
  + [Python开发实战系列(2)：Python爬虫](https://edu.aliyun.com/course/1369)
  + [Python开发实战系列(3)：Python web开发实战](https://edu.aliyun.com/course/1368)